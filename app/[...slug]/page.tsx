...
export default async function Page({ params }: { params: Promise<{ slug: string[] }> }) {
  ...
  if (path === "iletisim") return <section className="section container"><div className="section-heading"><p className="eyebrow">İletişim</p><h1>Servis için bize ulaşın</h1><p>{COMPANY.address}</p><p><Link className="text-link" href={COMPANY.phoneHref}>{COMPANY.phoneDisplay}</Link> · <Link className="text-link" href={COMPANY.whatsappUrl}>WhatsApp</Link></p><p>{COMPANY.hours} · <Link href={`mailto:${COMPANY.email}`}>{COMPANY.email}</Link></p></div><div className="form-layout"><div><Link className="button outline" href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(COMPANY.address)}`}>Yol Tarifi Al</Link></div><ServiceForm /></div></section>;
  const service = SERVICES.find(item => item.slug === path);
  if (!service) notFound();
