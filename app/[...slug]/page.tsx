...
function Contact() {
  ...
  return <section className="section container"><div className="section-heading"><p className="eyebrow">İletişim</p><h1>Servis için bize ulaşın</h1><p className="lead">İletişim talebiniz için e-posta, telefon veya WhatsApp kanalını kullanabilirsiniz.</p></div><div className="contact-card"><p><strong>E-posta:</strong> <a className="text-link" href={`mailto:${COMPANY.email}`}>{COMPANY.email}</a></p><p><strong>Telefon:</strong> <a className="text-link" href={COMPANY.phoneHref}>{COMPANY.phoneDisplay}</a></p><p><strong>WhatsApp:</strong> <a className="text-link" href={COMPANY.whatsappUrl}>WhatsApp üzerinden yazın</a></p><p><strong>Adres:</strong> {COMPANY.address}</p><p><strong>Çalışma saatleri:</strong> {COMPANY.hours}</p><a className="button outline" href={directionsUrl}>Yol Tarifi Al</a></div></section>;
}
...
function Service({ service }: { service: (typeof SERVICES)[number] }) {
  ...
  return <><section className="service-hero"><div className="container service-hero-grid"><div><p className="eyebrow">İzmir Bağımsız Teknik Servis</p><p className="breadcrumb"><a href="/">Ana Sayfa</a> / {service.name}</p><h1>İzmir {service.name}</h1><p className="lead">{service.description} Cihazınızdaki arıza belirtisi için telefon, e-posta veya WhatsApp üzerinden bize ulaşabilirsiniz.</p><div className="actions"><a className="button amber" href={COMPANY.phoneHref}>Hemen Ara</a><a className="button outline" href={COMPANY.whatsappUrl}>WhatsApp&apos;tan Yaz</a></div></div><div className="service-hero-image"><ServiceVisual visual={service.visual} priority /></div></div></section><section className="section container split"><div><h2>{service.name} hakkında</h2><p>Arıza, bakım veya onarım ihtiyacınız için cihaz ve yaşadığınız sorun hakkında temel bilgiyi telefon, e-posta veya WhatsApp üzerinden iletebilirsiniz.</p><h2>Yaygın sorunlar</h2><div className="chips"><span>Çalışmıyor</span><span>Ses yapıyor</span><span>Hata kodu veriyor</span><span>Program tamamlamıyor</span></div></div><div><h2>Servis bölgeleri</h2><p>İzmir&apos;in birçok ilçesinde hizmet verilmektedir. Beydağ, Kiraz ve Ödemiş kapsam dışıdır.</p><a href="/hizmet-bolgeleri" className="text-link">Hizmet bölgelerini incele →</a><h2 className="related-title">Diğer hizmetler</h2><div className="related-links">{related.map(item => <a key={item.slug} href={`/${item.slug}`}>{item.short} →</a>)}</div></div></section><section className="contact-section"><div className="container contact-section__inner"><div><p className="eyebrow">İletişim</p><h2>{service.name} için bize ulaşın</h2><p>Telefon, e-posta veya WhatsApp üzerinden doğrudan iletişime geçebilirsiniz.</p></div><div className="actions"><a className="button amber" href={COMPANY.phoneHref}>Hemen Ara</a><a className="button outline" href={`mailto:${COMPANY.email}`}>E-posta Gönder</a></div></div></section></>;
}
